...
async function extractPhonesFinal() {
  try {
    console.log('Telefon bilgileri çıkarılıyor (H sütunu mantığı ile)...\n');
    ...
    for (let row = 1; row <= worksheet.rowCount; row++) {
      ...
      if (customerCode) {
        ...
        // H sütununda (8. sütun) telefon numarası var mı kontrol et
        const phoneNumber = getText(rowData, 8);
        if (phoneNumber && validatePhone(phoneNumber)) {
          console.log(`  Telefon bulundu: ${phoneNumber}`);
          foundPhones++;
          ...
          console.log(`  - Geçersiz telefon: ${phoneNumber}`);
        } else {
          console.log(`  - Telefon bilgisi yok`);
        }
      }
    }
